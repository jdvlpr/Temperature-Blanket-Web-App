...
<script context="module">
  ...
  export const hash = derived(settings, ($settings) => {
    let hash = '&';
    hash += `${id}=`;
    hash += `${$settings.leftTarget}${$settings.rightTarget}(${$settings.dimensions}${CHARACTERS_FOR_URL_HASH.separator}${
  ...
  export const load = (hash) => {
    ...
    _settings.leftTarget = targets[0];
    _settings.rightTarget = targets[1];
...
<label class="label">
  ...
  <select
    class="select w-fit"
    id="smsq-left-params"
    bind:value={$settings.leftTarget}
  >
...
<label class="label">
  ...
  <select
    ...
    bind:value={$settings.rightTarget}
  >
    {#each targets as { id, label, icon }}
      <option value={id}>{icon} {label}</option>
    {/each}
