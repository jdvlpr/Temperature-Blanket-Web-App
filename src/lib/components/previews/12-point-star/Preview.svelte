...
<script lang="ts">
  ...
  runPreview(() => {
    const sections: TwelvePointStarSection[] = [];
      ...
      for (let row = 0; row < maxRows; row++) {
        ...
        if (dayIdxInMonth < actualDayCount) {
          ...
          color = (getColorInfo({
            param: twelvePointStarPreview.settings.selectedTargets[paramIdx],
            value,
          }).hex ?? '#000000') as NonNullable<Color['hex']>;
          isWeather = true;
        } else {
...
{:else}
  <svg
    ...
    onclick={(e) => {
      ...
      const isweatherAttr = e.target.getAttribute('data-isweather');
      const dayindexAttr = e.target.getAttribute('data-dayindex');
      if (isweatherAttr !== 'true') return;
      weather.currentIndex = +(dayindexAttr ?? 0);
